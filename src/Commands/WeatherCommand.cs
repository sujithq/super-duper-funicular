...
public class WeatherCommand : AsyncCommand<WeatherCommand.Settings>
{
    ...
    private static readonly HashSet<string> ValidAnalyses = new(StringComparer.OrdinalIgnoreCase)
    {
        "overview", "correlation", "patterns", "recommendations"
    };

    public override ValidationResult Validate(CommandContext context, Settings settings)
    {
        if (!ValidAnalyses.Contains(settings.Analysis))
        {
            return ValidationResult.Error($"Invalid analysis: '{settings.Analysis}'. Valid values are: overview, correlation, patterns, recommendations.");
        }
