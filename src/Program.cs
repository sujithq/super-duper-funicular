...
namespace SolarScope
{
    internal class Program
    {
        static async Task<int> Main(string[] args)
        {
            ...
            // Console.Out.Flush(); // Force flush console output
            ...
            // Console.WriteLine($"Parser result type: {parserResult.GetType().Name}");
            // Console.Out.Flush(); // Force flush console output

            return await parserResult.MapResult(
                (DashboardOptions opts) =>
                {
                    // Console.WriteLine("Dashboard command recognized!");
                    // Console.Out.Flush();
                    return ExecuteCommand(opts, async () => await new DashboardCommand().ExecuteAsync(opts));
                },
                ...
                errs =>
                {
                    // Console.WriteLine("Command parsing failed - showing help");
                    // Console.WriteLine($"Errors: {string.Join(", ", errs.Select(e => e.Tag.ToString()))}");
                    // Console.Out.Flush();
                    return Task.FromResult(DisplayHelpAndExit(parserResult, errs));
                }
            );
        }
